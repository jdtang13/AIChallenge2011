...
class Ants():
    ...
    def find_path(self, loc1, loc2): #go from loc1 to loc2
        ...
        row, col = loc1 #use each individual values
        end_row, end_col = loc2
        ...
        open_set = set()
        open_set.add(loc1) #open set initially contains the start
        closed_set = set()
        ...
        g_score[loc1] = 0
        h_score[loc1] = (abs(end_row-row) + abs(end_col - col))*10
        f_score[loc1] = g_score[loc1] + h_score[loc1]
        ...
        while open_set:
            ...
            cur_loc = min(open_set,key=lambda x:f_score[x])
            
            cur_row, cur_col = cur_loc

            neighbors = [(cur_row+1,cur_col),(cur_row-1,cur_col),(cur_row,cur_col-1),(cur_row,cur_col+1)]
            
            if cur_loc == loc2:
                return retrace_path(cur_loc)
            ...
            for myloc in neighbors:
                ...
                if myloc in closed_set:
                    continue
                
                tentative_g_score = g_score[cur_loc] + (abs(myrow-cur_row) + abs(mycol-cur_col))*10

                if myloc not in open_set:
                    open_set.add(myloc)
                    tentative_better = True

                elif tentative_g_score < g_score[myloc]:
                    tentative_better = True
                ...
                if tentative_better:
                    ...
                    g_score[myloc] = tentative_g_score
                    h_score[myloc] = (abs(end_row-myrow)+abs(end_col - mycol))*10
                    
                    f_score[myloc] = g_score[myloc] + h_score[myloc]
            
        #important: convert  locations into directions
                    
        index = 0
        while (index < len(locations)-1 ):
            directions.append(self.direction(locations[index], locations[index+1]) )
