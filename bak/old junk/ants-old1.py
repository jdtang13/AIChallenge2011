...
class Ants():
    ...
    def find_path(self, loc1, loc2): #go from loc1 to loc2
        ...
        parents = {} # dictionary
        #g_score = {} # cost from start to best known path
        #h_score = {} # heuristic cost estimate from path to end

        costs = {}

        open_set.add(cur_loc) #open set initially contains the start
        
        def retrace_path(loc1):
            directions.insert(0,loc1)
            if parents[loc1] == None:
                return
            retracePath(parents[loc1])
                    
        while len(open_set) is not 0:

            cur_loc = min(open_set,key=lambda x:costs[x])
            # syntax correct, but receives a key error
            
            if cur_loc == loc2:
                return retrace_path(cur_loc)
            
            open_set.delete(cur_loc)
            closed_set.add(cur_loc)
            
            for row in range(ants.rows):
                for col in range(ants.cols):
                    if (row,col) not in closed_set:
                        
                        costs[row,col] = (abs(end_row-row)+abs(end_col - col))*10
                        
                        if (row,col) not in open_set:
                            open_set.add(row,col)
                            
                        parents[row,col] = cur_loc
