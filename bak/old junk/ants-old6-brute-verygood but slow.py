...
class Ants():
    ...
    # use this: self.map[row][col] != WATER
    # use: self.destination(self, loc, direction)
    def find_path(self, loc1, loc2): #go from loc1 to loc2
        ...
        while cur_loc != loc2: # it looks like this loop never ends

            if len(open_set) == 0:
                cur_loc = loc1

            else:
                cur_loc = min(open_set,key=lambda x:costs[x]) #seems good
                
            cur_col, cur_row = cur_loc

            #neighbors are ill-defined, they need to wrap around
            ...
            for n in neighbors[:]:
                ...
                if not self.passable(n): #nodes are invalid if they are surrounded by water
                    neighbors.remove(n) # seems good

                if n in open_set or n in closed_set: # this prevents repeats IT WORKS!!!
                    neighbors.remove(n)
        ...
        #important: convert  locations into directions
                    
        index = 0 
        
        while (index < len(locations) - 1 ): #sometimes it goes opposite
