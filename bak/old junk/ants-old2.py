...
class Ants():
    ...
    def find_path(self, loc1, loc2): #go from loc1 to loc2
        ...
        def retrace_path(locx): # this may actually be fine
            directions.insert(0,locx)
            if parents[locx] == None:
                return
            
            retrace_path(parents[locx])

            
        while len(open_set) is not 0: # Bug: this is never zero?
