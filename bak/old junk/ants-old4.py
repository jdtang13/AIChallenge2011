...
class Ants():
    ...
    def find_path(self, loc1, loc2): #go from loc1 to loc2
        ...
        open_set = set()
        open_set.add(cur_loc) #open set initially contains the start
        closed_set = set()
...
##        h_score[loc1] = (abs(end_row-row)+abs(end_col - col))*10
        ...
        def retrace_path(locx):
            ...
            while parents[locx] is not None:
                locx = parents[locx]
                path.append(locx)
        ...
        while open_set: # Bug: length of open set is never zero?

            for rowTemp in range(self.rows):
                for colTemp in range(self.cols):
                    myloc = (rowTemp,colTemp)
                    
                    if myloc not in closed_set:
                        
                        costs[myloc] = (abs(end_row-rowTemp)+abs(end_col - colTemp))*10
