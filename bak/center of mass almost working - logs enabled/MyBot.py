...
##            new_row, new_col = new_loc
            ...
            for ant_loc in ants.my_ants():
                ...
                for d in dlist:
                    ...
                    if self.i[ant_loc] > 0:
                        new_rib = float(((self.i[ant_loc])*rib + row)/(self.i[ant_loc] + 1))
                        new_cib = float(((self.i[ant_loc])*cib + col)/(self.i[ant_loc] + 1)) #update the ci's and ri's
                        
                    else:
                        new_rib = row
                        new_cib = col
                        
                    di =  float(abs(row - rib) + abs(col - cib)) #di at i
...
##                        if not ants.passable(ants.destination(new_loc_tmp, tempdir)):
##                            continue
