...
##        
##        getLogger().debug("In do_turn: COM of previous turn: row_prev %f, col_prev %f", row_prev, col_prev)
##
##        if ( self.turn_number == 1):
##            self.com_all_ants = [row, col]
##        else:
##            k = 2
##            row_prev, col_prev = self.com_all_ants
##            row = (row_prev + row*(k-1))/k # Yi k^i weighted average
##            col = (col_prev + col*(k-1))/k
##            self.com_all_ants = [row, col]
...
##            getLogger().debug("previous distance is %d, new distance is %d", dist_prev, dist)
            
            if dist > dist_prev:
##              getLogger().debug("%d is diffuse value before", self.diffuse[ants.destination(ant_loc,direction)])
              self.diffuse[ants.destination(ant_loc,direction)] += self.food_value/10000
...
##              do_move_direction(ant_loc, my_dir)
##              bool_tmp = True
##              break
##
##          return bool_tmp
...
##            possibles.append((row-1,col-1))
        ...
        def diffuse((row,col)):
          ...
          if (not ants.passable((row,col)) ):
            self.diffuse[(row,col)] = water_value
            if (not ants.passable((row,col))) and (row,col) not in self.waters:
              self.waters.append((row,col))
##              getLogger().debug("added to waters %d,%d ", row, col)
            ...
            for (water_row, water_col) in self.waters:
            ...
            for (hill_row, hill_col) in self.hills:
              ...
              #self.diffuse[loc] += score(hill_value,hill_decay,(row,col),(hill_row,hill_col))
